...
import (
	...
	"go.opentelemetry.io/collector/pdata/external/cmd/pdatagen/external"
)
...
func main() {
	for _, fp := range internal.AllFiles {
		f, err := os.Create("./pdata/external/generated_" + fp.Name + ".go")
		check(err)
		_, err = f.WriteString(fp.GenerateFile())
		check(err)
		check(f.Close())
		f, err = os.Create("./pdata/external/generated_" + fp.Name + "_test.go")
		check(err)
